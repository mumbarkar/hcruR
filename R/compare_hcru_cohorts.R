...
compare_hcru_cohorts <- function(hcru_summary, cohort) {
  hcru_summary <- left_join(hcru_summary, cohort, by = "person_id")

  hcru_summary |>
    group_by(.data[["cohort_id"]], .data[["domain"]], .data[["time_window"]]) |>
    summarise(
      mean_count = mean(.data[["event_count"]], na.rm = TRUE),
      median_count = median(.data[["event_count"]], na.rm = TRUE),
